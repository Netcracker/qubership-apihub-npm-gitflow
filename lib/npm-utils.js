...
function changeLernaProjectVersion(version, branchName = null) {
    return new Promise((resolve) => {
        ...
        exec(`npx lerna version ${version} --no-push --no-private --no-git-tag-version ${allowBranchOption} --yes`, err => {
            handleError(err);
...
/**
 * Gets the incremented patch version from the provided version
 * 
 * @param {string} version - The current version
 * @returns {Promise<string>} A promise that resolves with the incremented patch version
 */
function getIncrementedPatchVersion(version) {
    return new Promise((resolve) => {
        // Extract the semantic version parts
        ...
        const incrementedPatchVersion = `${major}.${minor}.${patch + 1}`;        
        resolve(incrementedPatchVersion);
    });
}
